...
\midrule
\textsc{OfferTransition} & \textit{AssessAndTransition}
...
Museum visitors exhibit diverse pacing preferences, but empirical studies consistently show frustration when guides rush through exhibits without substantive discussion \citep{falk-dierking-2016-museum,bitgood2013attention}. A functional transition policy must balance visitor autonomy (respecting signals to move on) with pedagogical responsibility (ensuring adequate content delivery). Early training revealed agents offering transitions after single-fact explanations, producing shallow coverage and low cumulative novelty rewards.

We implement a two-part solution grounded in the notion of \textit{content readiness} for transitions. First, we introduce a \textbf{transition insufficiency penalty}:

\[
r^{\text{trans}}_t = 
\begin{cases}
-0.20 & \text{if agent chose OfferTransition and } |\mathcal{F}_{\text{current}}| < 2 \\
0 & \text{otherwise}
\end{cases}
\]

where $\mathcal{F}_{\text{current}}$ denotes the set of facts already shared about the current exhibit. The threshold of 2 facts represents a minimal substantive engagement: identifying the object and providing at least one contextual detail. This is consistent with museum interpretation guidelines that recommend ``layered'' content delivery, starting with basic identification and building to deeper themes \citep{serrell2015exhibit}.\\

Second, we model \textbf{visitor acceptance of transitions as probabilistic}, conditioned on exhibit completion. When the agent selects \textsc{OfferTransition}, the simulator computes a transition success probability:

\[
p_{\text{accept}} = 
\begin{cases}
0.20 & \text{if completion} = 0\% \\
0.50 & \text{if } 0\% < \text{completion} < 33\% \\
0.80 & \text{if } 33\% \leq \text{completion} < 67\% \\
0.95 & \text{if completion} \geq 67\%
\end{cases}
\]

where completion is the ratio of facts mentioned to total available facts for the exhibit. If the transition succeeds (Bernoulli draw with probability $p_{\text{accept}}$), the visitor moves to the target exhibit and responds positively. If it fails, the visitor remains at the current exhibit and responds with confusion (e.g., ``But we've barely discussed this piece—why move on already?''), yielding low dwell on the subsequent turn.\\

This design is motivated by models of visitor agency in informal learning: people need a sense of control and adequate ``dwell time'' to process information before moving \citep{falk-dierking-2016-museum,bitgood2013attention}. The probabilistic acceptance mirrors findings that even well-timed transitions are sometimes declined due to individual interest, so the agent must learn robustness to rejection. The staged probabilities (20\% $\rightarrow$ 95\%) operationalize the intuition that visitors become more receptive to transitions as content coverage increases. Empirically, this mechanism reduced premature transitions from 35\% of all transitions to under 12\%, while maintaining high overall transition success rates (avg. 78\%) by the end of training.\\

Finally, to prevent \textit{transition spam}—repeated \textsc{OfferTransition} attempts without intervening content—we add a \textbf{spam penalty}:

\[
r^{\text{spam}}_t = -0.10 \times \max(0, n_{\text{consec}} - 2)
\]

where $n_{\text{consec}}$ is the count of consecutive \textsc{OfferTransition} actions. This allows up to two sequential transition attempts (e.g., visitor declines, agent rephrases) but escalates penalties for further attempts without explaining. The penalty resets to zero when any non-transition option is chosen. Similar mechanisms appear in dialogue management literature as ``action repetition penalties'' to enforce natural pacing \citep{young2013pomdp,casanueva2018feudal}.\\

\subsubsection{Conclude Bonus: Systematic Coverage Incentive}

The \textsc{Conclude} option ends the session, so its timing directly affects total content coverage and visitor experience. Early policies tended toward two failure modes: concluding too early (after 1--2 exhibits) when the cumulative novelty reward was still low, or never concluding (exhausting the turn budget) because immediate per-turn rewards from continued explanation outweighed the uncertain value of stopping. Both violate the spirit of a guided tour, which should systematically cover multiple exhibits and end gracefully rather than abruptly truncating or dragging on.\\

Educational dialogue systems address this through \textit{goal-completion bonuses} that reward achieving session-level objectives \citep{koedinger2012knowledge,rollinson2014framework}. In museum settings, systematic coverage—touching on multiple exhibits rather than deep-diving on one—aligns with visitor preferences for ``overview'' tours and learning objectives around breadth of exposure \citep{falk-dierking-2016-museum,bitgood2013attention}. We operationalize this through a \textbf{conclude bonus} that scales with exhibit diversity:

\[
r^{\text{conclude}}_t = 
\begin{cases}
0.20 \times |\mathcal{E}_{\text{covered}}| & \text{if agent chose Conclude} \\
0 & \text{otherwise}
\end{cases}
\]

where $\mathcal{E}_{\text{covered}} = \{ e \in \mathcal{E} : |\mathcal{F}_e| > 0 \}$ is the set of exhibits for which at least one fact has been mentioned. With 8 exhibits in the knowledge graph, the maximum bonus is $+1.6$, comparable to 6--7 turns of high-engagement explanation (each worth $\approx 0.25$ from dwell + novelty). This makes concluding after broad coverage competitively attractive relative to extending the session on fewer exhibits.\\

The linear scaling (0.20 per exhibit) is a simplification; a sublinear function (e.g., logarithmic) might better reflect diminishing marginal value of additional exhibits. However, early experiments with sublinear scaling led to premature conclusions after 3--4 exhibits, as the per-exhibit marginal gain dropped too quickly. The linear form empirically yielded the best balance: agents learned to cover 5--7 exhibits on average (out of 8) before concluding, consistent with visitor attention spans for guided tours (15--20 minutes, or roughly 12--15 dialogue turns in our setting).\\

This design is further supported by work on hierarchical RL for long-horizon tasks: explicit ``goal-achievement'' rewards at option termination improve credit assignment when intermediate rewards are sparse \citep{bacon2017option,peng2017composite}. The \textsc{Conclude} option is the natural semantic endpoint for the session, and attaching a coverage-based bonus there directly signals the session-level objective to the learning agent.

\subsubsection{Combined Reward Function}

Integrating these extensions with the base components from the formal problem definition, the full per-turn reward is:

\[
R_t = r^{\text{eng}}_t + r^{\text{nov}}_t + r^{\text{resp}}_t + r^{\text{trans}}_t + r^{\text{spam}}_t + r^{\text{conclude}}_t
\]

where $r^{\text{eng}}_t = \text{dwell}_t$ (engagement from gaze), $r^{\text{nov}}_t = 0.15 \times |\text{new facts at } t|$ (knowledge novelty), and the three extensions add cooperative responsiveness, content-pacing constraints, and session-level coverage goals. The weights are hand-tuned based on early training: engagement and novelty provide the primary signal (typical per-turn range 0.0--0.8), while responsiveness, transition control, and conclude bonus act as corrective shaping (range $-0.2$ to $+1.6$, but sparse—most turns these are zero). This balance keeps the agent sensitive to moment-to-moment engagement while respecting higher-level conversational and pedagogical structure.
...
\begin{enumerate}
    ...
    \item \textbf{Generates utterance text} using either template-based expansion or an LLM prompt (mistral medium for higher quality mode and mistral small for faster responses).
...
As described in the transition control subsection, the simulator models transitions as probabilistic events conditioned on exhibit completion. When the agent selects \textsc{OfferTransition}, the simulator:
...
For efficiency, the simulator operates in two modes. \textbf{Template mode} (fast) uses hand-authored templates per response type (e.g., ``What's special about the [AOI]?'' for questions, ``That's interesting!'' for acknowledgments), with slot-filling for the current AOI and exhibit. This runs in $<10$ms per turn and is used during high-throughput training. \textbf{LLM mode} (detailed) uses Mistral-small or Mistral-medium with a structured prompt that includes the agent's last utterance, current AOI, persona, and desired response type. The prompt instructs the LLM to generate a 1--2 sentence natural response that reflects the visitor's state (engaged, confused, curious). This produces more diverse and contextually appropriate utterances at the expense of longer turn times during training.\\



Reinforcement learning requires an environment that provides informative feedback. For museum dialogue, this means a visitor simulator that (i) generates natural language responses, (ii) produces gaze/engagement signals, and (iii) makes both contingent on agent behavior quality. A simulator that responds uniformly regardless of agent actions would provide no learning signal for dialogue quality; conversely, one that is too sensitive or brittle would produce noisy, high-variance gradients. We aim for a middle ground: clear, interpretable response patterns that reward good behavior (answering questions, staying on-topic) and penalize poor behavior (deflection, off-topic rambling).
...
The hierarchical policy selects high-level options and low-level subactions, but these must be realized as concrete utterances through the LLM-based dialogue planner. This subsection details how subactions are executed, how knowledge-graph grounding is enforced, and how hallucination is detected and penalized.\\
...
The LLM (GPT-3.5-turbo) then generates a response like: ``The dove you see at the top represents the Holy Spirit [AN\_002], a common symbol in Annunciation scenes. It signifies divine presence.''
...
\begin{enumerate}
    ...
    \item Constructs a prompt with header: \texttt{OPTION: OfferTransition | SUBACTION: AssessAndTransition | TARGET: [e*]}.
...
\subsection{Summary: From Theory to Trainable System}

The extensions documented in this section operationalize the formal hierarchical framework into a practical, trainable dialogue agent. The reward function now captures three layers of objectives: moment-to-moment engagement (dwell), cooperative information exchange (responsiveness, transition pacing), and session-level pedagogical goals (coverage, systematic conclude). The simulator provides contingent feedback that reflects realistic visitor behavior—rewarding good dialogue moves with high engagement and positive responses, penalizing poor moves with confusion and disengagement. Grounding mechanisms (fact verification, prompt contracts, action masking) ensure the agent stays faithful to the knowledge base and produces structurally coherent dialogue strategies.\\

Together, these mechanisms enable the hierarchical policy to learn long-horizon conversational strategies from online interaction: the manager learns which options to deploy and when to switch based on engagement and intent signals, while intra-option policies learn how to realize those strategies through structured prompting of the LLM. The result is a system that balances the flexibility and fluency of large language models with the coherence and goal-directedness of learned hierarchical control.
