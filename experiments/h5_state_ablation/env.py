"""
H5 Environment Variant: State Ablation
...
class H5StateAblationEnv(MuseumDialogueEnv):
    """
    Environment variant for H5 hypothesis: state ablation using dialogue acts.
    ...
    def __init__(self, *args, **kwargs):
        """Initialize H5 environment variant."""
        ...
        print(f"[H5] State ablation: {new_obs_dim}-d (vs {149}-d baseline)")
        print(f"[H5] Dialogue act representation: 8-d probability distribution (soft labels)")
    
    def _get_obs(self):
        """
        Construct observation with dialogue-act state (H5 variant).
